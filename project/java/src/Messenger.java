...
public class Messenger {
   private static String cListID;//ID of contact list
   private static String blistID;//ID of block list
   ...
   public static void AddToContact(Messenger esql){
      ...
      if (uExist > 0){
         String query2 = String.format("INSERT INTO USER_LISTCONTAINS(list_id, list_member) VALUES ('%s', '%s')", cListID, uname);
   ...
   public static void ListContacts(Messenger esql){
      ...
      try{
         ...
         while(rs.next()){
            ...
            int uExist = esql.executeQuery(query);
            if(uExist > 0){//do i need this check?
               
            }
            else{
               System.out.println("User "+ uname + " isn't in contacts!");
            }
         }
   ...
   public static void NewMessage(Messenger esql){
      // Your code goes here.
      try{
         if(userExists(esql, someUser )){
            String body = null;
            System.out.print("\t\tInsert message body: ");
            body = in.readLine();
            Timestamp t = new Timestamp(system.currentTimeMillis() );
            String stat = "SENT";
            String query = String.format("INSERT INTO message(senderid, receiverid, body, sendtime, deletestatus, status ) VALUES ('%s%', '%s%', '%s%','%s%','%s%','%s%')", current_user, someUser, body, t, d, stat);
            esql.executeUpdate(query);
